...
if (isset($_POST['action']) && $_POST['action'] == "model")
    ...
    echo '<option value="" selected="selected">اختيار الفئة</option>';
...
if (isset($_POST['action']) && $_POST['action'] == "sub_model")
    ...
    echo '<option value="" selected="selected">اختار النوع</option>';
    foreach ($result as $value)
        {
...
if (isset($_POST['action']) && $_POST['action'] == "app_model")
    ...
    if (@$result)
        ...
        {
        echo '<option value="" selected="selected">لا يوجد سيارات معتمدة</option>';
        }
    }
...
//    print_r($var1);
    ...
    $html = '<div class="col-sm-2 fields-left">
          ...
          <div class="fields-right-inner">
           ...
           <h1><span>الصنع:</span></h1>
           <p><span>الفئة:</span></p>
           <p><span>النوع:</span></p>
            <p><span>الصنف:</span></p>
            
            <p><span>اللون الخارجي:</span></p>
            
             <p><span>اللون الداخلي:</span></p>
           <p><span>السعر:</span> </p>
          <p><span>الضمان:</span> </p>
          <p class="acc"><span>التسارع (0-100):</span> </p>
         <p><span>قاطع:</span> </p>
          <p><span>السرعة القصوى:</span> </p>
          <p><span>سعة المحرك:</span> </p>
          <p><span>نوع مغير السرعة:</span></p>
          <p><span>فتحة سقف:</span></p>
          <p><span>الكاميرا:</span></p>
           <p><span>نظام الملاحة:</span></p>
            <p><span>حوادث:</span></p>
             <p><span>السنة:</span></p>
           <p><span>انتهاء باقة الخدمة:</span></p>
            <p><span>الضمان:</span></p>
...
//            echo 'sada';
		...
		<div class="fields-right-inner">
        <h1><span>الصنع:</span>' . $maker . '</h1>
        <p><span>الفئة:</span>' . $model . '</p>            
        <p><span>النوع:</span>' . $sub_model . '</p>            
        <p><span>الصنف:</span>' . $bodyType . '</p>  
        <p><span>اللون الخارجي:</span>' . $exterior_color . '</p>
        <p><span>اللون الداخلي:</span>' . $interior_color . '</p>
        <p><span>السعر:</span>QAR ' . number_format($row['price']) . '</p>
        <p><span>الضمان:</span>' . $year . '</p>
        <p><span>التسارع (0-100):</span>' . $row['acceleration'] . ' seconds</p>
        <p><span>قاطع:</span>' . $row['milage'] . '</p>
        <p><span>السرعة القصوى:</span>' . $row['top_speed'] . ' km/hr</p>
        <p><span>سعة المحرك:</span>' . $row['engineSize'] . ' litres</p> 
        <p><span>نوع مغير السرعة:</span>' . $transmission . '</p>
        <p><span>فتحة سقف:</span>' . $roof . '</p>
        <p><span>الكاميرا:</span>' . $camara . '</p>
        <p><span>نظام الملاحة:</span>' . $GPS . '</p>
        <p><span>حوادث:</span>' . $accident . '</p>
        <p><span>السنة:</span>' . $row['year'] . '</p>
        <p><span>انتهاء باقة الخدمة:</span>' . $row['service_package'] . '</p>
        <p><span>الضمان:</span>' . $difference->y . ' years, ' . $difference->m . ' months, ' . $difference->d . ' days' . '</p>
        <p><span>حصان:</span>' . $row['h_power'] . ' HP</p>
        </div></div>';
//            $aaa = 12;
            }
        else if ($var2 == $var )
   ...
   <p><span>GPS:</span>' . $GPS . '</p>
    <p><span>Accident:</span>' . $accident . '</p>
     <p><span>السنة:</span>' . $row['year'] . '</p>
    <p><span>انتهاء باقة الخدمة:</span>' . $row['service_package'] . '</p>
        <p><span>الضمان:</span>' . $difference->y . ' years, '
                    . $difference->m . ' months, '
                    . $difference->d . ' days' . '</p>
...
if (isset($_POST['action']) && $_POST['action'] == "compare")
    ...
    while ($row = mysql_fetch_array($run_query))
        ...
        <h1>' . $get->get_single_field('maker', 'name', 'id', $row['maker_id']) . ' ' . $get->get_single_field('model', 'model_name', 'model_id', $row['model']) . ' ' . $get->get_single_field('sub_model', 'sub_model_name', 'sub_id', $value['sub_model']) . '</h1>
        <p><span>السعر:</span>QAR ' . number_format($row['price']) . '</p>
        <p><span>الضمان:</span>' . $row['Warranty'] . '</p>
        <p><span>التسارع (0-100):</span>' . $row['acceleration'] . ' seconds</p>
        <p><span>السرعة القصوى:</span>' . $row['top_speed'] . ' km/hr</p>
        <p><span>سعة المحرك:</span>' . $row['engineSize'] . ' litres</p> 
        <p><span>حصان:</span>' . $row['h_power'] . ' HP</p>
        ...
        <h1>' . $get->get_single_field('maker', 'name', 'id', $row['maker_id']) . ' ' . $get->get_single_field('model', 'model_name', 'model_id', $row['model']) . ' ' . $get->get_single_field('sub_model', 'sub_model_name', 'sub_id', $value['sub_model']) . '</h1>
        <p><span>السعر</span>QAR ' . number_format($row['price']) . '</p>
        <p><span>الضمان</span>' . $row['الضمان'] . '</p>
        <p><span>التسارع (0-100)</span>' . $row['acceleration'] . ' seconds</p>
        <p><span>السرعة القصوى</span>' . $row['top_speed'] . ' km/hr</p>
        <p><span>سعة المحرك</span>' . $row['engineSize'] . ' litres</p> 
        <p><span>حصان</span>' . $row['h_power'] . ' HP</p>
