...
class UsedCarsImageController extends GxController
    {
    ...
    public function accessRules()
        ...
        return array(
            ...
            array('allow',
                'actions' => array('create', 'toggle', 'update', 'search', 'admin', 'delete'),
                'users' => array('@'),
            ),
//				array('allow', 
...
//					),
    ...
    public function actionView($id)
        {
        $model = $this->loadModel($id, 'UsedCarsImage');
    ...
    public function actionToggle()
        {
        $model = $this->loadModel($_REQUEST['pk'], 'UsedCarsImage');
//        print_r($model);
        $criteria = new CDbCriteria();
        $criteria->Compare('car_id', $model->car_id);
        $all_images = UsedCarsImage::model()->findAll($criteria);
        foreach ($all_images as $key => $val)
            {
            $new_model = $this->loadModel($val->id, 'UsedCarsImage');
            $new_model->state_id = User::STATE_INACTIVE;
            $new_model->save();
            }
//            die();
...
//        else 
        ...
        $model->save();
        $model_car = $this->loadModel($model->car_id, 'UsedCar'); 
        $model_car->banner_image = $model->image;
        if ($model_car->save())
            {
//            
            return array(
...
//}
    ...
    public function actionCreate()
        {
        $model = new UsedCarsImage;

        $this->performAjaxValidation($model, 'used-cars-image-form');

        if (isset($_POST['UsedCarsImage']))
            {
//            print_r($_POST);
//            echo "das";
//            print_r($_FILES);
//            die();
            $model->setAttributes($_POST['UsedCarsImage']);
    ...
    public function actionUpdate($id)
        ...
        if (isset($_POST['UsedCarsImage']))
            ...
            if ($model->save())
                {
                $this->redirect(array('/usedCar/view', 'id' => $model->car_id));
                }
            }
        $this->updateMenuItems($model);
    ...
    public function actionDelete($id)
        ...
        if (Yii::app()->getRequest()->getIsPostRequest())
            {
            $this->loadModel($id, 'UsedCarsImage')->delete();
    ...
    public function actionIndex()
        ...
        $dataProvider = new CActiveDataProvider('UsedCarsImage');
    ...
    public function actionSearch()
        ...
        if (isset($_GET['UsedCarsImage']))
            {
            $model->setAttributes($_GET['UsedCarsImage']);
    ...
    public function actionAdmin()
        {
        $model = new UsedCarsImage('search');
        $model->unsetAttributes();
        $this->updateMenuItems($model);

        if (isset($_GET['UsedCarsImage'])) $model->setAttributes($_GET['UsedCarsImage']);
    ...
    protected function updateMenuItems($model = null)
        ...
        if ($model == null) $model = new UsedCarsImage();
