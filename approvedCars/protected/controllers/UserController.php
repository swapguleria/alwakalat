...
//        $this->layout = '//layouts/column1';
        $model1 = new User('create');
        $makers = Maker::model()->findAll();

        $makers_array = array();
        foreach ($makers as $key => $val)
            {
            $makers_array[] = $val['name'];
            }
        $this->performAjaxValidation($model1, 'user-form');
...
//            echo '</per>';
            $model1->setAttributes($_POST ['User']);
            $model1->saveUploadedFile($model1, 'logo');
            if (@$_POST['options']) $model1->makers = implode(',', $_POST['options']);
            $user = User::getUserByEmail($model1->email);
            if (!$user)
                {

                $model1->state_id = User::STATE_ACTIVE;
                if ($model1->role_id == null) $model1->role_id = User::ROLE_USER;

                if ($model1->save())
                    {
                    $this->redirect(array('admin'));   //redirect to action that you want.
//                    $model1->saveUploadedFile($model1, 'image_file');
//                    if (isset($model1->password) && $model1->setPassword($model1->password, $model1->password_2))
//                        {
//                        $login = new LoginForm ();
//                        $login->username = $model1->first_name;
//                        $login->email = $model1->email;
...
//                    else
//                        {
//                        $model1->addError('password', 'Password not set');
//                        }
                    }
                }
            else
                {
                echo $model1->addError('email', 'Email already exist.');
                }
            }

        $this->updateMenuItems($model1);
        $this->render('signup', array(
            'model' => $model1,
            'makers_array' => $makers_array
        ));
        }
...
//                    }
    ...
    public function actionUpdate($id = null)
        ...
        if (isset($_POST ['User']))
            {
            $model->setAttributes($_POST ['User']);
            if (@$_POST['options']) $model->makers = implode(',', $_POST['options']);
            else $model->makers = '';
...
//                }
            ...
            $image2 = $model->saveUploadedFile($model, 'logo');
...
// $this->menu[] = array('label'=>Yii::t('app', 'Manage'), 'url'=>array('admin'), 'visible'=> Yii::app()->user->isAdmin,'icon'=>'fa fa-wrench ');
            ...
            case 'admin' :
                    {
//$this->menu[] = array('label'=>Yii::t('app', 'List'), 'url'=>array('index'),'icon'=>'icon-th-list ');
                    $this->menu[] = array('label' => Yii::t('app', 'Create Dealer'), 'url' => array('signup'), 'icon' => 'fa fa-plus ', 'visible' => Yii::app()->user->isAdmin);
                    }
                break;
