...
abstract class GxActiveRecord extends CActiveRecord {
	 ...
	 *
	 * @param integer $n
	 *        	The number value. This is used to support plurals. Defaults to 1 (means singular).
	 ...
	 *        	The relation, attribute or class property name.
	 *        	This method supports chained relations in the form of "post.author.name".
	 * @param integer $n
	 *        	The number value. This is used to support plurals.
	 ...
	 * @param boolean $useRelationLabel
	 *        	Whether to use the relation label for the FK attribute.
		...
		foreach ( $relNames as $relName ) {
			...
			if (! isset ( $relations [$relName] )) {
				...
				if ($relIndex === $countRelNames) {
					...
					throw new InvalidArgumentException ( Yii::t ( 'giix', 'The attribute "{attribute}" should be the last name.', array (
							'{attribute}' => $relName 
					) ) );
				}
			}
	 ...
	 *        	The names of the attributes to be selected.
	 *        	Optional. If not specified, the {@link representingColumn} will be used.
	 * @param boolean $withPk
	 *        	Specifies if the primary keys will be selected.
	 ...
	 *
	 * @param GxActiveRecord|array $model
	 *        	A model or an array of models.
	 * @param boolean $forceString
	 *        	Whether pk values on composite pk tables
	 ...
	 *
	 * @param mixed $pk
	 *        	The PK value or array of PK values.
	 ...
	 *        	The relation data in the format returned by {@link GxController::getRelatedData}.
	 ...
	 *        	Whether to perform validation before saving the record.
	 * @param boolean $batch
	 *        	Whether to try to do the deletes and inserts in batch.
	 *        	While batches may be faster, using active record instances provides better control, validation, event support etc.
	 ...
	 *
	 * @param GxActiveRecord|array $models
	 *        	A model or an array of models.
	 ...
	 *        	</ul>
	 * @param boolean $runValidation
	 *        	Whether to perform validation before saving all the records.
	 ...
	 *
	 * @param string|GxActiveRecord $modelClass
	 *        	The model class name or a model instance.
	 * @param string|CDbColumnSchema $column
	 *        	The column.
...
/* 	public function scopes() {
		return array (
				...
				), // . ' OR to_list= '. Yii::app()->user->model->username),
				'latest' => array (
	...
	public function truncate($content, $maxlength = 500) {
		if (strlen ( $content ) < $maxlength)
			return $content;
		
		preg_match ( '/^([^.!?]*[\.!?]+){0,3}/', strip_tags ( $content ), $mystring );
		$content = $mystring [0];
		return $content;
	}
...
//                        exit();
	...
	public function formatDateTime($attribute, $bShowTime = false) {
		$format = "F jS, Y ";
		if ($bShowTime)
			$format = "F jS, Y h:m:s a";
	...
	public static function rrmdir($dir) {
		if (is_dir ( $dir )) {
			...
			if ($dir != Yii::app ()->basePath . '/../assets' and $dir != Yii::app ()->runtimePath and $dir != Yii::app ()->basePath . '/..' . UPLOAD_PATH) {
				if (rmdir ( $dir ))
